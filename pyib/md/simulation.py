...
class SingleParticleSimulation:
    ...
    Attributes:
        potential (openmm.CustomExternalForce): Underlying 2D potential energy surface.
    ...
    Arguments:
        ...
        chkfile (str): File to write checkpoints to (default = "./chk_state.pkl").
        trajfile (str): File to write trajectory data to (default = "./traj.dat").
        energyfile (str): File to write energies to (default = "./energies.dat").
    """
    def __init__(self,
    ...
    def __call__(self,
        ...
        for i in tqdm(range(nsteps)):
            ...
            # Store positions
            if i % trajevery == 0:
                self.write_trajectory(trajfile, i)

            # Store energy
            if i % energyevery == 0:
                self.write_energies(energyfile, i)
    ...
    def write_trajectory(self, ofilename, i):
        t = i * self.timestep / unit.femtosecond * unit.picosecond
    ...
    def write_energies(self, ofilename, i):
        t = i * self.timestep / unit.femtosecond * unit.picosecond
